...
if __name__ == "__main__":
    ...
    parser.add_argument('--clustering_iters', default=20, type=int, help='iteration of clustering stage')
    ...
    print(x.shape)
    # print(x)
    pos = np.array(data_mat['obsm']['spatial'])
    
    num_rows = data_mat['X'].shape[0]
    y = np.arange(1, num_rows + 1)
    
    data_mat.close()
    
    # Find non-"NA" values
    # Apply the filter to all arrays (assuming they have the same number of rows)
    f = np.where(y.astype(str) != "NA")[0]
    print(f.shape, pos.shape, x.shape, y.shape)
    y = y[f]
    x = x[f, :]
    pos = pos[f, :]
    ...
    if args.final_labels != -1:
        df = pd.DataFrame({
            'Id': range(len(y_pred)),  # Generate an ID column starting from 0 to len(y_pred) - 1
            'Label': y_pred            # Store the predicted labels
        })

        df.to_csv(args.save_dir + "/" + args.final_labels + "_" + str(args.run) + ".csv", index=False)
